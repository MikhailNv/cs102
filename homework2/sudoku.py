...
def solve(grid: List[List[str]]) -> Optional[List[List[str]]]:
    ...
    """
    def verification(grid: List[List[str]]):
        find=find_empty_positions(grid)
        if not find:
            return True
        else:
            new_set=find_possible_values(grid, (find[0], find[1]))
            arr=list(new_set)
            if (len(arr))==0:
                return False
            else:
                for i in arr:
                    grid[find[0]][find[1]]=i
                    if verification(grid) is True:
                        return True
                    else:
                        grid[find[0]][find[1]] = '.'
    verification(grid)
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    for h in range (len(solution)):
        for k in range (len(solution)):
            col=set(get_col(solution, (h, k)))
            row=set(get_row(solution, (h, k)))
            block=set(get_block(solution, (h, k)))
...
def generate_sudoku(N: int) -> List[List[str]]:
    ...
    while N>0:
        ...
        if (grid[number1][number2]!= "."):
            grid[number1][number2]="."
            N-=1
    return (grid)
